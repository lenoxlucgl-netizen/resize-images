		...
		async function loadExistingFiles() {
			...
			try {
				...
				data.files.forEach(f => {
					...
					item.style = 'display: flex; flex-direction: column; padding: 10px; background: var(--white); border: 1px solid var(--line); border-radius: 4px;';
					item.innerHTML = `<span style="font-family:'DM Mono',monospace; font-size:11px; color:var(--ink); word-break: break-all; margin-bottom: 8px;"><a href="/api/files/object/${encodeURIComponent(f.Key)}?bucket=${encodeURIComponent(targetBucket)}" target="_blank" style="color:var(--ink); text-decoration:none;">${f.Key}</a></span>
									<button type="button" class="del-btn" style="align-self: flex-end; background:#e5534b; color:#fff; border:none; padding:4px 8px; border-radius:4px; font-size:10px; cursor:pointer;">Elimina</button>`;
